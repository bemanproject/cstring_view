...
configure beman.cstring_view to fetch them automatically via CMake FetchContent.
...
```bash
cmake -LH -S . -B build | grep "BEMAN_CSTRING_VIEW" -C 2
```
...
This is required so that users of `beman.cstring_view` can use
`find_package(beman.cstring_view)` to locate the library.
